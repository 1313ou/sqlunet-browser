...
object Spanner {
     ...
     * prepareTextViewForClickableSpan(textView)
     ...
     */
    @ReturnThis
    fun SpannableStringBuilder.insertTag(position: Int, tag: CharSequence): SpannableStringBuilder {
        val insert = "$tag\n$EOEXPANDEDSTRING"
        insert(position, insert)
        val mark = position + tag.length + 1
        setSpan(HiddenSpan(), mark, mark + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        return this
    }
     ...
     */
    @ReturnThis
    fun SpannableStringBuilder.collapse(position: Int): SpannableStringBuilder {
        delete(position, find(position, EOEXPANDEDSTRING))
        return this
    }
     ...
     */
    fun SpannableStringBuilder.find(start: Int, delimiter: Char): Int {
        var i = start
        while (i < length) {
            if (this[i] == delimiter) {
                return i + 1
