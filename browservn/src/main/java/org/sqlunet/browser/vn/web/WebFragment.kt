...
class WebFragment : Fragment() {
    ...
    private fun load() {
        ...
        // settings sources
        val enable = org.sqlunet.browser.vn.VnSettings.getAllPref(AppContext.context)
        ...
        // load the contents
        model!!.loadData(WebDocumentStringLoader(AppContext.context, pointer, pos, type, data, enable, xml))
    }
    ...
    private fun docsToString(
        ...
        if (xml) {
            ...
            if (BuildConfig.DEBUG) {
                ...
                writeLog(false, AppContext.context, null, wnDomDoc, vnDomDoc, pbDomDoc)
                writeLog(data, false, AppContext.context, LogUtils.DOC_LOG)
                // Log.d(TAG, "output=\n$data")
            }
        }
