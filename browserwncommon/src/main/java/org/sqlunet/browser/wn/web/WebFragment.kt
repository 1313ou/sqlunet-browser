...
class WebFragment : Fragment() {
    ...
    private fun load() {
        ...
        // settings sources
        var mask = 0
        if (org.sqlunet.browser.wn.WnSettings.getWordNetPref(AppContext.context)) {
            mask = org.sqlunet.browser.wn.WnSettings.Source.WORDNET.set(mask)
        }
        if (org.sqlunet.browser.wn.WnSettings.getBncPref(AppContext.context)) {
    ...
    private fun docsToString(
        ...
        if (xml) {
            ...
            if (BuildConfig.DEBUG) {
                ...
                writeLog(false, AppContext.context, null, wnDomDoc, bncDomDoc)
                writeLog(data, false, AppContext.context, LogUtils.DOC_LOG)
                // Log.d(TAG, "output=\n$data")
            }
        }
