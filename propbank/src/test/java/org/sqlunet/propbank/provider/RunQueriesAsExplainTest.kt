...
class RunQueriesAsExplainTest {
    ...
    private fun uriToCode(providerUri: String): Int {
        return when (providerUri) {
            PropBankContract.PbRoleSets.URI1 -> PropBankControl.PBROLESET1
    ...
    companion object {
        ...
        private fun toSql(code: Int, providerSql: ContentProviderSql): String {
            val r = queryMain(code, uriLast, providerSql.projection, providerSql.selection, providerSql.selectionArgs) ?: throw IllegalArgumentException("Illegal query code: $code")
