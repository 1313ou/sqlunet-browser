...
class SqLiteDialect
{
	...
	static final String PropBankRolesQueryFromRoleSetId = //
			"SELECT ${roles.roleid},${roles.roledescr},${argtypes.argtypeid},${funcs.func},${vnroles.vnroleid} " + //
					"FROM ${rolesets.table} " + //
	...
	static final String PropBankExamplesQueryFromRoleSetId = //
			"SELECT ${examples.exampleid},${examples.text},${rels.rel}," + //
					"GROUP_CONCAT(DISTINCT ${args.argtypeid}||'~'||(CASE WHEN ${funcs.func} IS NULL THEN '*' ELSE ${funcs.func} END)||'~'||REPLACE(${roles.roledescr},',','﹐')||'~'||(CASE WHEN ${vnroles.vnrole} IS NULL THEN '*' ELSE ${vnroles.vnrole} END)||'~'||${args.arg}) " + //
					"FROM ${rolesets.table} " + //
