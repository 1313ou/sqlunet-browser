...
class VnClassSenseMap
{
	 ...
	 */
	private VnClassSenseMap(final String className, final long classId, final Long wordId, final Long synsetId, final Integer senseNum, final String senseKey, final float quality, final String groupings)
	{
		super();
	 ...
	 */
	static public List<VnClassSenseMap> make(final SQLiteDatabase connection, final long wordId, final Long synsetId)
	{
		final List<VnClassSenseMap> result = new ArrayList<>();
		...
		try
		{
			...
			while (query.next())
			{
				...
				result.add(new VnClassSenseMap(className, classId, wordId, synsetSpecificFlag ? synsetId : null, sensenum, sensekey, quality, groupings));
			}
		}
		finally
		{
