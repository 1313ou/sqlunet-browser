...
class WebFragment : Fragment() {
                        ...
                        when (type) {
                            ProviderArgs.ARG_QUERYTYPE_ALL -> if (pointer != null) {
                                if (pointer is XSelectorPointer) {
                                    ...
                                    val collocationId = pointer.id
                                    snDomDoc = SyntagNetImplementation().queryCollocationDoc(db, collocationId)
                                } else if (pointer is CollocationSelectorPointer) {
                                    val selectorPointer = pointer
                                    val wordId = selectorPointer.getWordId()
                    ...
                    if ("word" == name) {
                        ...
                        when (name) {
                            ...
                            "collocationid" -> {
                                ...
                                pointer = SnCollocationPointer(id)
                            }
    ...
    private fun docsToString(
        ...
        if (xml) {
            ...
            if (snDomDoc != null) {
                rootDomDoc.documentElement.appendChild(rootDomDoc.importNode(snDomDoc.firstChild, true))
            }
            if (bncDomDoc != null) {
