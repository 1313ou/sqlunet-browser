...
public class ExpandableListFragment extends Fragment implements ExpandableListView.OnChildClickListener, ExpandableListView.OnGroupCollapseListener, ExpandableListView.OnGroupExpandListener
{
	...
	public void setEmptyText(CharSequence text)
	{
		...
		{
			throw new IllegalStateException("Can't be used with a custom content view"); //$NON-NLS-1$
		}
	...
	private void ensureList()
	{
		...
		if (root == null)
		{
			throw new IllegalStateException("Content view not yet created"); //$NON-NLS-1$
		}
		...
		else
		{
			...
			this.mExpandableListContainer = root.findViewById(INTERNAL_LIST_CONTAINER_ID);
			...
			if (!(rawExpandableListView instanceof ExpandableListView))
			{
				...
				{
					throw new RuntimeException("Your content must have a ListView whose id attribute is " + "'android.R.id.list'"); //$NON-NLS-1$ //$NON-NLS-2$
				}
				throw new RuntimeException("Content has view with id attribute 'android.R.id.list' " + "that is not a ListView class"); //$NON-NLS-1$ //$NON-NLS-2$
			}
	...
	public void onContentChanged()
	{
		...
		if (this.mExpandableList == null)
		{
			throw new RuntimeException("Your content must have a ExpandableListView whose id attribute is " + "'android.R.id.list'"); //$NON-NLS-1$ //$NON-NLS-2$
		}
