...
object ColorsTest : TestCase() {
    ...
    internal class IllegalColorPair(private val res: String, private val resId: Int, private val index: Int, private val backColor: Int, private val foreColor: Int, private val defaultBackColor: Int, private val defaultForeColor: Int, private val contrast: Double) : Exception() {
        override val message: String
            get() = String.format(
                "%d %s [%02d] %s on %s (default %s on %s) with contrast %f ", resId, res, index, colorToString(foreColor), colorToString(backColor), colorToString(
                    defaultForeColor
                ), colorToString(defaultBackColor), contrast
            )
    }
}