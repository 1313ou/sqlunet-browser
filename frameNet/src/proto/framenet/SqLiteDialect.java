...
class SqLiteDialect
{
	...
	static public final String FrameNetFrameQuery = //
			"SELECT ${as_frames}.${frames.frameid},${as_frames}.${frames.frame},${as_frames}.${frames.framedefinition}, " + //
					"(SELECT GROUP_CONCAT(${semtypes.semtypeid}||':'||${semtypes.semtype}||':'||${semtypes.semtypedefinition}, '|') " + // GROUP_CONCAT DELIMITER
					...
					"(SELECT GROUP_CONCAT(${frames_related.frame2id}||':'||${as_related_frames}.${frames.frame}||':'||${framerelations.relation}, '|') " + // GROUP_CONCAT DELIMITER
	...
	static public final String FrameNetFEQueryFromFrameId = //
			"SELECT ${fetypes.fetypeid},${fetypes.fetype},${fes.feid},${fes.fedefinition},${fes.feabbrev},${coretypes.coretype},GROUP_CONCAT(${semtypes.semtype}, '|') AS ${a_types},${coretypes.coretypeid} = 1 AS ${iscorefe},${fes.coreset} " + // GROUP_CONCAT DELIMITER
					"FROM ${frames.table} " + //
	...
	static public final String FrameNetLayerQueryFromAnnoSetId = //
			"SELECT ${layers.layerid},${layertypes.layertype},${annosets.annosetid},${layers.rank},GROUP_CONCAT(${labels.start}||':'||${labels.end}||':'||${labeltypes.labeltype}||':'||CASE WHEN ${labelitypes.labelitype} IS NULL THEN '' ELSE ${labelitypes.labelitype} END, '|') " + // GROUP_CONCAT DELIMITER
					"FROM " + //
